...
// Combined Panic, UnsafeMath, and 512Math for testing.
// Source: https://github.com/0xProject/0x-settler (MIT license).
...
contract Math512Harness {
    function addUint256(uint256 a, uint256 b) external pure returns (uint256 hi, uint256 lo) {
        uint512 lhs = alloc();
        uint512 rhs = alloc();
        uint512 out = alloc();
        lhs.from(a);
    ...
    function addUint512(uint256 aHi, uint256 aLo, uint256 bHi, uint256 bLo) external pure returns (uint256 hi, uint256 lo) {
        uint512 lhs = alloc();
        uint512 rhs = alloc();
        uint512 out = alloc();
        lhs.from(aHi, aLo);
    ...
    function addUint512Scalar(uint256 hiIn, uint256 loIn, uint256 scalar) external pure returns (uint256 hi, uint256 lo) {
        uint512 base = alloc();
        uint512 out = alloc();
        base.from(hiIn, loIn);
        out.oadd(base, scalar);
        (hi, lo) = out.into();
    }

    function mulUint256(uint256 a, uint256 b) external pure returns (uint256 hi, uint256 lo) {
        uint512 out = alloc();
        out.omul(a, b);
        (hi, lo) = out.into();
    }

    function gtUint512(uint256 aHi, uint256 aLo, uint256 bHi, uint256 bLo) external pure returns (bool) {
        uint512 lhs = alloc();
        uint512 rhs = alloc();
        lhs.from(aHi, aLo);
        rhs.from(bHi, bLo);
    ...
    function compareToScalar(uint256 hiIn, uint256 loIn, uint256 scalar)
        external
        pure
        returns (bool isGreater, bool isEqual)
    {
        uint512 value = alloc();
        value.from(hiIn, loIn);
        isGreater = value.gt(scalar);
        isEqual = value.eq(scalar);
    }

    function modUint512(uint256 hiIn, uint256 loIn, uint256 modulus) external pure returns (uint256) {
        uint512 value = alloc();
        value.from(hiIn, loIn);
        return value.mod(modulus);
    }

    function roundTripExternal(uint256 hiIn, uint256 loIn) external pure returns (uint256 hi, uint256 lo) {
        uint512 value = alloc();
        value.from(hiIn, loIn);
        uint512_external memory ext = value.toExternal();
        uint512 back = alloc();
        back.from(ext);
        (hi, lo) = back.into();
    }
